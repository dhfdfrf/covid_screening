...
class _LesionPriorBranch(nn.Module):
    """Image-derived lesion prior from intensity, edges, and local contrast."""
    ...
    def forward(self, image: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        feat = self.encoder(torch.cat([gray, edge, local_mean, local_contrast], dim=1))
        return feat, self.head(feat)


class _UncertaintyAwareCalibrator(nn.Module):
    """Small residual logit calibrator controlled by a zero-initialized scale."""

    def __init__(self, feat_ch: int, hidden_ch: int = 32):
        super().__init__()
        sobel_x = torch.tensor(
            [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]],
            dtype=torch.float32,
        ...
        self.net = nn.Sequential(
            nn.Conv2d(feat_ch + 5, hidden_ch, 3, padding=1, bias=False),
    ...
    def forward(
        ...
        image_edge = self._edge(image.mean(dim=1, keepdim=True))
        ...
        correction = self.net(
            torch.cat([feat, prob, uncertainty, pred_edge, prior_prob, image_edge], dim=1)
        )
        return torch.tanh(self.gamma) * correction


class BoundaryAwareTransUNet2D_v17(nn.Module):
    """v17: stable v12/v14 backbone plus prior-supervised residual calibration."""
    ...
    def __init__(
        ...
        self.prior_branch = _LesionPriorBranch(channels=prior_channels)
        ...
        self.calibrator = _UncertaintyAwareCalibrator(c1, hidden_ch=c1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        base_logits = coarse + self.refine(d1, coarse)
        calibration = self.calibrator(d1, base_logits, prior_logits, x)
        seg_logits = base_logits + calibration
...
def build_transunet2d_v17(
    in_channels: int = 1,
    out_channels: int = 1,
    use_boundary_head: bool = True,
    use_deep_supervision: bool = True,
):
    return BoundaryAwareTransUNet2D_v17(
...
if __name__ == "__main__":
    ...
    print("[Train] calibration:", out["calibration"].abs().mean().item())
