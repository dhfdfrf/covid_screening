...
v9成果: Dice=0.7743 IoU=0.6700 Prec=0.7813 Rec=0.8281 (全面最优)
v9问题: TTA未被使用 (推理速度1.23it/s = 未开启)

v10 = v9架构原封不动 + TTA默认开启

TTA机制:
  - self.training=True  → 正常前向 (训练)
  - self.training=False → 4-flip平均 (验证+推理)
  
  训练脚本如果validation变慢4倍, 可加:
    model.enable_tta = False  # 在validation循环前
    model.enable_tta = True   # validation结束后恢复 (可选)
  
  或者不管它, 让validation也用TTA (更准确的模型选择, 只是慢一点)
"""
...
class BoundaryAwareTransUNet2D_v10(nn.Module):
    """v10 = v9架构 + TTA默认开启.
    
    TTA控制:
      model.enable_tta = True   # 默认, eval时4-flip平均
      model.enable_tta = False  # 关闭TTA, 普通单次推理
    """
...
def build_transunet2d_v10(
    in_channels: int = 1,
    out_channels: int = 1,
    use_boundary_head: bool = True,
    use_deep_supervision: bool = True,
):
    return BoundaryAwareTransUNet2D_v10(
...
if __name__ == "__main__":
    model = build_transunet2d_v10(in_channels=1, out_channels=1)
    ...
    model.eval()
    out_tta = model(x)
    print("[Eval+TTA] seg:", out_tta["seg"].shape)

    # 关闭TTA
    model.enable_tta = False
    out_no = model(x)
    print("[Eval-TTA] seg:", out_no["seg"].shape)

    diff = (out_tta["seg"] - out_no["seg"]).abs().mean().item()
    print(f"TTA effect (mean logit diff): {diff:.4f}")
